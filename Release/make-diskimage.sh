...
rm -f "${DMG_TEMP_NAME}"
hdiutil makehybrid -o "${DMG_TEMP_NAME}" -hfs -hfs-volume-name "${VOLUME_NAME}" -hfs-openfolder "${SRC_FOLDER}" "${SRC_FOLDER}"
...
# unmount to force writing of .DS_Store
echo "Unmounting disk image..."
hdiutil detach "${DEV_NAME}"

# compress image
echo "Compressing disk image..."
hdiutil convert -format UDZO "${DMG_TEMP_NAME}" -o "${DMG_DIR}/${DMG_NAME}"
...
if [ ! -z "${EULA_RSRC}" -a "${EULA_RSRC}" != "-null-" ]; then
        ...
        /Developer/Tools/Rez -a sla.r -o "${DMG_DIR}/${DMG_NAME}"
