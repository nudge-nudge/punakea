...
-(void) dealloc
{
	...
	while( (fdNum = [enny nextObject]) )
	{
    	if( close( [fdNum integerValue] ) == -1 )
            lcl_log(lcl_cnntagging,lcl_vWarning,@"dealloc: Couldn't close file descriptor (%d)",errno);
    }
...
-(void) removePathFromQueue: (NSString*)path
{
    ...
    AT_SYNCHRONIZED( self )
    {
        ...
        fd = [[watchedFDs objectAtIndex: idx] integerValue];
	...
	if( close( fd ) == -1 )
        lcl_log(lcl_cnntagging,lcl_vWarning,@"removePathFromQueue: Couldn't close file descriptor (%d)",errno);
}
...
-(void)		watcherThread: (id)sender
{
    ...
    while( keepThreadRunning )
    {
		...
		NS_HANDLER
			lcl_log(lcl_cnntagging,lcl_vError,@"Error in UKKQueue watcherThread: %@",localException);
		NS_ENDHANDLER
	...
	if( close( theFD ) == -1 )
		lcl_log(lcl_cnntagging,lcl_vError,@"release: Couldn't close main kqueue (%d)", errno);
